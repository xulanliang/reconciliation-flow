...
import com.yiban.rec.bill.parse.util.FtpsUtil;
 ...
 *
 */
public abstract class AbstractFTPBankBillParser<T> extends AbstractBillParser<ThirdBill>{
	...
	protected FTPConfig getFtpConfig() {
		...
		String isFtps = ProConfigManager.getValueByPkey(entityManager, ProConstants.bankFtpIsFtps,"false");
		return new FTPConfig(host, port, username, password, ftpPath, savePath,"true".equals(isFtps));
	}
	/**
	 * 获取ftp文件名称
	 * @param date
	...
	protected File downLoadFtpFile(FTPConfig ftpConfig,String fileName){
		...
		File file = null;
		if(ftpConfig.isFtps()){
			file = FtpsUtil.downloadFtpsFile(ftpConfig.getHost(), ftpConfig.getUsername(), ftpConfig.getPassword(), ftpConfig.getPort(), ftpConfig.getFtpPath(), ftpConfig.getSavePath(), fileName);
		}else{
			file = FtpUtil.downloadFtpFile(ftpConfig.getHost(), ftpConfig.getUsername(), ftpConfig.getPassword(), ftpConfig.getPort(), ftpConfig.getFtpPath(), ftpConfig.getSavePath(), fileName);
		}
