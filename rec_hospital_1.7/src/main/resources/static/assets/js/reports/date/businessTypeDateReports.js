NB.ns("app.admin").ReportsPayBusinessTypeDate= (function() {
	// 表格
	var tableObj = $("#businessTypeDateReportsDataTable");
	var baseUrl = "/admin/date/businessType";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#businessTypeDateReportsSearchForm");
	// 时间控件
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				var starttime = "";
				var endtime = "";
				var rangeTime = $("#businessTypeTime").val();
				if(rangeTime){
				...
				var name = starttime +"至"+ endtime+treeName+"日期业务类型汇总报表";
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#businessTypeDateOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#businessTypeDateOrgSelect").parent().parent().parent().show();
				}else{
					$("#businessTypeDateOrgSelect").parent().parent().parent().hide();
				}
				
	           	var treeName = summaryByPayBusinessTypeTree.getText;
	           	setTableTitleHide(tableObj,"日期业务类型汇总报表",beginTime,endTime,treeName);
           }
       });
	}
	...
	function search(way) {
		...
		tableObj.bootstrapTable('refreshOptions', {
			  ...
			  queryParams:function(params){
				    var queryObj =formObj.serializeObject();
				    var rangeTime = queryObj['businessTypeTimeRange'];
				    if(rangeTime){
	            ...
	            onPreBody:function(data){
	            	$("#searchBusinessTypeDateReportsBtn").button('loading');
	            },
	            onLoadSuccess:function(data){
	            	$("#searchBusinessTypeDateReportsBtn").button("reset");
		        }
		});
		...
		setTableTitleHide(tableObj,"日期业务类型汇总报表",starttime,endtime,treeName);
	}
	...
	function initDate(type, format, beginTime, endTime) {
		...
		var beginDivDom = $('#businessTypeTime').parent('div');
		$('#businessTypeTime').remove();
		beginDivDom.append('<input type="text" class="form-control" style="width: 220px;" onmousemove="this.style.cursor=\'pointer\';" id="businessTypeTime" name="businessTypeTimeRange" readonly=\"readonly\"/>');
		var startLayDate = laydate.render({
			elem : '#businessTypeTime',
			btns: ['confirm'],
			theme : '#A9BCF5',
			type: type,
			value: rangeTime,
