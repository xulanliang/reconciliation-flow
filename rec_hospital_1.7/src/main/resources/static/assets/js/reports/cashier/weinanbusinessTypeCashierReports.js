	...
	function exportList(){
		...
		bootbox.confirm('确定执行此操作?',function(r){
            if (r){
        		...
        		var url = baseUrl+'/dcExcel/weinan?' + where;
...
//		amount = row.allAmount - row.registerAmount - row.makeAppointmentAmount - row.payAmount - row.clinicAmount - row.prepaymentForHospitalizationAmount;		
	...
	function otherAmountSub(val, row, index){
		var amount;
		amount = accAdd(row.registerAmountSub, row.makeAppointmentAmountSub);
		amount = accAdd(amount, row.payAmountSub)
		amount = accAdd(amount, row.clinicAmountSub)
		amount = accAdd(amount, row.prepaymentForHospitalizationAmountSub)
//		amount = row.allAmount - row.registerAmount - row.makeAppointmentAmount - row.payAmount - row.clinicAmount - row.prepaymentForHospitalizationAmount;		
	...
	function otherAmountAdd(val, row, index){
		var amount;
		amount = accAdd(row.allAmount, - row.registerAmount);
		amount = accAdd(amount,- row.makeAppointmentAmount);
		amount = accAdd(amount,- row.payAmount)
...
//		amount = row.allAmount - row.registerAmount - row.makeAppointmentAmount - row.payAmount - row.clinicAmount - row.prepaymentForHospitalizationAmount;		
	...
	return {
		...
		otherAmountAdd:otherAmountAdd,
		otherAmountSub:otherAmountSub,
