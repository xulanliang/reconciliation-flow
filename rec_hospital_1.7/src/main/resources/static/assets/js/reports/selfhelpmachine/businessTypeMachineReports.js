NB.ns("app.admin").ReportsBusinessTypeMachine= (function() {
	// 表格
	var tableObj = $("#businessTypeMachineReportsDataTable");
	var baseUrl = "/admin/selfhelpmachine/businessType";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#businessTypeMachineReportsSearchForm");
	// 时间控件
	var datePluginId = "businessTypeMachineTime"; 
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				...
				var name = starttime +"至"+ endtime+treeName+"自助机业务类型汇总报表";
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#businessTypeMachineOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#businessTypeMachineOrgSelect").parent().parent().parent().show();
				}else{
					$("#businessTypeMachineOrgSelect").parent().parent().parent().hide();
				}
	           	
	          	var treeName = summaryByPayBusinessTypeTree.getText;
	          	setTableTitleHide(tableObj,"自助机业务类型汇总报表",beginTime, endTime,treeName);
           }
       });
	}
	...
	function search(th) {
		...
		var treeName = summaryByPayBusinessTypeTree.getText;
		setTableTitleHide(tableObj,"自助机业务类型汇总报表",starttime,endtime,treeName);
	}
