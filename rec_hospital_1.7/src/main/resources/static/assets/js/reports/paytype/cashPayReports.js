NB.ns("app.admin").ReportsCashPay= (function() {
	// 表格
	var tableObj = $("#cashPayReportsDataTable");
	var baseUrl = "/admin/paytype/cashpay";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#cashPayReportsSearchForm");
	// 时间控件
	var datePluginId = "cashPayReportsTime"; 
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				...
				var name = starttime +"至"+ endtime+treeName+"支付方式现金汇总报表";
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#cashPayOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#cashPayOrgSelect").parent().parent().parent().show();
				}else{
					$("#cashPayOrgSelect").parent().parent().parent().hide();
				}
	           	
	           	var treeName = summaryByPayBusinessTypeTree.getText;
	           	setTableTitleHide(tableObj,"支付方式现金汇总报表",beginTime, endTime,treeName);
	...
	function search(th) {
		...
		var treeName = summaryByPayBusinessTypeTree.getText;
		setTableTitleHide(tableObj,"支付方式现金汇总报表",starttime,endtime,treeName);
	}
