NB.ns("app.admin").ReportsHealthPay= (function() {
	// 表格
	var tableObj = $("#healthPayReportsDataTable");
	var baseUrl = "/admin/paytype/healthpay";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#healthPayReportsSearchForm");
	// 时间控件
	var datePluginId = "healthPayReportsTime"; 
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				...
				var name = starttime +"至"+ endtime+treeName+"支付方式医保汇总报表";
				// 获取当前打开的tag页名
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#healthPayOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#healthPayOrgSelect").parent().parent().parent().show();
				}else{
					$("#healthPayOrgSelect").parent().parent().parent().hide();
				}
	           	
	           	var treeName = summaryByPayBusinessTypeTree.getText;
	    		setTableTitleHide(tableObj,"支付方式医保汇总报表",beginTime, endTime,treeName);
           }
       });
	}
	...
	function search(th) {
		...
		var treeName = summaryByPayBusinessTypeTree.getText;
		setTableTitleHide(tableObj,"支付方式医保汇总报表",starttime,endtime,treeName);
	}
