NB.ns("app.admin").ReportsJuhePaySource= (function() {
	// 表格
	var tableObj = $("#juhePaySourceReportsDataTable");
	var baseUrl = "/admin/syssource/juhepay";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#juhePaySourceReportsSearchForm");
	// 时间控件
	var datePluginId = "juhePaySourceTime"; 
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				...
				var name = starttime +"至"+ endtime+treeName+"系统来源聚合支付汇总报表";
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#juhePaySourceOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#juhePaySourceOrgSelect").parent().parent().parent().show();
				}else{
					$("#juhePaySourceOrgSelect").parent().parent().parent().hide();
				}
				
	           	var treeName = summaryByPayBusinessTypeTree.getText;
	           	setTableTitleHide(tableObj,"系统来源聚合支付汇总报表",beginTime, endTime,treeName);
           }
       });
	}
