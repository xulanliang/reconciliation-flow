NB.ns("app.admin").ReportsOutpatientdept= (function() {
	// 表格
	var tableObj = $("#outpatientdeptReportsDataTable");
	var baseUrl = "/admin/businesstype/outpatientdept";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#outpatientdeptReportsSearchForm");
	// 时间控件
	var datePluginId = "outpatientdeptReportsTime"; 
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
            if (r){
            	...
            	var name = starttime +"至"+ endtime+treeName+"收费员门诊汇总报表";
            	// 获取当前打开的tag页名
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#outpatientdeptOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#outpatientdeptOrgSelect").parent().parent().parent().show();
				}else{
					$("#outpatientdeptOrgSelect").parent().parent().parent().hide();
				}
	           	
	           	var treeName = summaryByPayBusinessTypeTree.getText;
	           	setTableTitleHide(tableObj,"门诊汇总报表",beginTime, endTime,treeName);
           }
       });
	}
	...
	function search(th) {
		...
		var treeName = summaryByPayBusinessTypeTree.getText;
       	setTableTitleHide(tableObj,"门诊汇总报表",starttime, endtime,treeName);
	}
