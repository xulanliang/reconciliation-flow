NB.ns("app.admin").ReportsCashPaySource= (function() {
	// 表格
	var tableObj = $("#cashPaySourceReportsDataTable");
	var baseUrl = "/admin/syssource/cashpay";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#cashPaySourceReportsSearchForm");
	// 时间控件
	var datePluginId = "cashPaySourceTime"; 
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				...
				var name = starttime +"至"+ endtime+treeName+"系统来源现金汇总报表";
				// 获取当前打开的tag页名
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#cashPaySourceOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#cashPaySourceOrgSelect").parent().parent().parent().show();
				}else{
					$("#cashPaySourceOrgSelect").parent().parent().parent().hide();
				}
				
	           	var treeName = summaryByPayBusinessTypeTree.getText;
	           	setTableTitleHide(tableObj,"系统来源现金汇总报表",beginTime, endTime,treeName);
           }
       });
	}
	...
	function search(th) {
		...
		var treeName = summaryByPayBusinessTypeTree.getText;
       	setTableTitleHide(tableObj,"系统来源现金汇总报表",starttime, endtime,treeName);
	}
