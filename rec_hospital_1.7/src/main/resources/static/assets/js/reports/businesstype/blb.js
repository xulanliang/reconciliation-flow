NB.ns("app.admin").ReportsBlb= (function() {
	// 表格
	var tableObj = $("#blbReportsDataTable");
	var baseUrl = "/admin/businesstype/blb";
	// 请求路径
	var apiUrl = baseUrl + '/data';
	// 表单
	var formObj = $("#blbReportsSearchForm");
	// 时间控件
	var datePluginId = "blbReportsTime";
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				...
				var name = starttime +"至"+ endtime+treeName+"(病历本/口罩)汇总报表";
				// 获取当前打开的tag页名
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
			...
			success:function(data){
				...
				summaryByPayBusinessTypeTree = $("#blbOrgSelect").ztreeview({
					name: 'name',
					key: 'code',
					//是否
					clearable:true,
				...
				if(length && length>1){
					$("#blbOrgSelect").parent().parent().parent().show();
				}else{
					$("#blbOrgSelect").parent().parent().parent().hide();
				}

				var treeName = summaryByPayBusinessTypeTree.getText;
				setTableTitleHide(tableObj,"(病历本/口罩)汇总报表",beginTime, endTime,treeName);
			}
		});
	}
	...
	function otherAcount(val, row, index){
		var count;
		count = row.allAcount - row.registerAcount - row.makeAppointmentAcount - row.payAcount - row.clinicAcount - row.prepaymentForblbAcount;
		if(isNaN(count)){
			count = 0;
		}
	...
	function otherAmount(val, row, index){
		var amount;
		amount = row.allAmount - row.registerAmount - row.makeAppointmentAmount - row.payAmount - row.clinicAmount - row.prepaymentForblbAmount;
		if(isNaN(amount)){
			amount = 0;
		}
		if(row.terminalNo == "合计"){
