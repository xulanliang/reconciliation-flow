NB.ns("app.admin").ReportsPayTypeCashier = (function() {
	// 表格
	var tableObj = $("#payTypeCashierReportsDataTable");
	var baseUrl = "/admin/cashier/payType";
	// 请求路径
	var apiUrl = baseUrl + '/data';
	// 表单
	var formObj = $("#payTypeCashierReportsSearchForm");
	// 时间控件
	var rangeTimeObj = $("#payTypeCashierTime");
				...
				.confirm(
						...
						function(r) {
							if (r) {
								...
								var name = starttime + "至" + endtime + treeName
										+ "收费员支付方式汇总报表";
								var orgCode = summaryByPayBusinessTypeTree.getVal;
								// 获取当前打开的tag页名
								var workSheetName = $(
										".main-sidebar .active .active .menu-open span")
										.html();

								var where = "orgName=" + treeName
										+ "&beginTime=" + starttime
										+ "&endTime=" + endtime + "&fileName="
										+ name + "&workSheetName="
										+ workSheetName + "&orgCode=" + orgCode;
	...
	function initOrgTree(orgNo, beginTime, endTime) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				summaryByPayBusinessTypeTree = $("#summaryByPayTypeOrgSelect")
						.ztreeview({
							name : 'name',
							key : 'code',
							// 是否
				...
				summaryByPayBusinessTypeTree.updateCode(data[0].id,
						data[0].code);
				if ((orgNo != "" && orgNo != null && orgNo != undefined)) {
					for (var i = 0; i < data.length; i++) {
						if (orgNo == data[i].code) {
							summaryByPayBusinessTypeTree.updateCode(data[i].id,
									data[i].code);
						}
					}
				}
				...
				if (length && length > 1) {
					$("#summaryByPayTypeOrgSelect").parent().parent().parent()
							.show();
				} else {
					$("#summaryByPayTypeOrgSelect").parent().parent().parent()
							.hide();
				...
				var treeName = summaryByPayBusinessTypeTree.getText;
				setTableTitleHide(tableObj, "收费员支付方式汇总报表", beginTime, endTime,
						treeName);
			}
		});
	}
	...
	function search(th) {
		...
		var treeName = summaryByPayBusinessTypeTree.getText;
		setTableTitleHide(tableObj, "收费员支付方式汇总报表", starttime, endtime, treeName);
	}

	function otherAcount(val, row, index) {
		var count;
		count = row.allAcount - row.registerAcount - row.makeAppointmentAcount
				- row.payAcount - row.clinicAcount
				- row.prepaymentForHospitalizationAcount;
		if (isNaN(count)) {
			count = 0;
		}
	...
	function otherAmount(val, row, index) {
		var amount;
		amount = row.allAmount - row.registerAmount - row.makeAppointmentAmount
				- row.payAmount - row.clinicAmount
		...
		if (row.businessType == "合计") {
			var html = "<span style='font-weight: bold;'>"
					+ new Number(amount).toFixed(2) + "</span>";
			return html;
		}
	...
	function initDate(beginTime, endTime) {
		...
		var startLayDate = laydate.render({
			...
			max : nowDate.getTime(),
			ready : function(date) {
				var layym = $(".laydate-main-list-0 .laydate-set-ym span")
						.eq(0).attr("lay-ym").split("-")[1];
