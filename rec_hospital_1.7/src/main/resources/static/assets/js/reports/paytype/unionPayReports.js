NB.ns("app.admin").ReportsUnionPay= (function() {
	// 表格
	var tableObj = $("#unionPayReportsDataTable");
	var baseUrl = "/admin/paytype/unionpay";
	// 请求路径
	var apiUrl = baseUrl + '/data'; 
	// 表单
	var formObj = $("#unionPayReportsSearchForm");
	// 时间控件
	var datePluginId = "unionPayReportsTime"; 
	...
	function exportList(){
		bootbox.confirm('确定执行此操作?',function(r){
			if (r){
				...
				var name = starttime +"至"+ endtime+treeName+"支付方式银联汇总报表";
	...
	function initOrgTree(orgNo,beginTime, endTime){
		...
		$.ajax({
           ...
           success:function(data){
        	   ...
        	   summaryByPayBusinessTypeTree = $("#unionPayOrgSelect").ztreeview({
		       			name: 'name',
		       			key: 'code', 
		       			//是否
		       			clearable:true,
        	   ...
        	   summaryByPayBusinessTypeTree.updateCode(data[0].id,data[0].code);
				...
				if(length && length>1){
					$("#unionPayOrgSelect").parent().parent().parent().show();
				}else{
					$("#unionPayOrgSelect").parent().parent().parent().hide();
				}
	           	
	           	var treeName = summaryByPayBusinessTypeTree.getText;
	    		setTableTitleHide(tableObj,"支付方式银联汇总报表",beginTime, endTime,treeName);
           }
       });
	}
	...
	function search(th) {
		...
		var treeName = summaryByPayBusinessTypeTree.getText;
		setTableTitleHide(tableObj,"支付方式银联汇总报表",starttime,endtime,treeName);
	}
