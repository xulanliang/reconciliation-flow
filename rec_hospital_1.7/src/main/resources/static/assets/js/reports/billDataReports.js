...
NB.ns("app.admin").ReportsPayBusinessType = (function () {
    // 表格
    var tableObj = $("#billDataReportsDataTable");
    var baseUrl = "admin/billData/";
    // 请求路径
    ...
    function exportList() {
        ...
        bootbox.confirm('确定执行此操作?', function (r) {
            if (r) {
                ...
                var name = starttime + "至" + endtime + treeName + "渠道数据报表";
                // 获取当前打开的tag页名
                // var workSheetName = $("#editor-tabs-menu .active span").html();
                var workSheetName = "渠道数据报表";
        ...
        tableObj.bootstrapTable('refreshOptions', {
            ...
            onLoadSuccess: function (data) {
                ...
                if (data.rows.length > 1) {
                    var colName = 'Cashiera';
                    var colCount = 12;
                    for (var j = 0; j < data.rows.length; j++) {
                        var row = data.rows[j];
                        if (j == data.rows.length - 1) {
                            tableObj.bootstrapTable('mergeCells', {
                                index: j,
                                field: colName,
                                colspan: colCount
                            });
                            remark = remark + row.Cashiera + "）";
                        } else if (j == data.rows.length - 2) {
                            remark ="（渠道合计金额：" + row.payAmountBill + "元，" + "His合计金额：" + row.payAmountHis + "元。"
                            // $("#billSumAmount").text("（渠道合计金额：" + row.alipayBill + "元");
                            // $("#HisSumAmount").text("His合计金额：" + row.alipayHis + "元）");
                        }
                    }
                }
                $("#remark").text(remark);
