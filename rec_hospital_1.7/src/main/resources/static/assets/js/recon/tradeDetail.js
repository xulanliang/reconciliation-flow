...
//		tradePaySource();
	...
	//柜员下拉
	function tradeDetailDeviceInfoData(){
		/*$.ajax({
            url:"/admin/deviceInfo/deviceInfos?orgNo="+tradeTree.getVal,
            contentType:"application/json",
            dataType:"json",
            success:function(msg){
            	changeSelectData(msg);
		...
		$.ajax({
            ...
            success:function(msg){
            	for(var i=0;i<msg.length;i++){
            		$("#deviceNo").append("<option value='"+msg[i].deviceNo+"'>"+msg[i].deviceNo+"</option>");
            	}
            }
        });
	}*/
		...
		$.ajax({
           ...
           success:function(msg){
        	 //这个树形下拉会在和其同级位置生成一个默认隐藏的input输入框以提供表单数据
			   ...
			   selectTypeData();
			   //柜员下拉
			   /*tradeDetailDeviceInfoData();*/
//			   collect(tradeTree.getVal);
//        	   collect(tradeTree.getVal);
//        	   var startDate=startobj.val()+" 00:00:00";
//        	   var endDate=endobj.val()+" 23:59:59";
//        	   var queryObj  = {orgCode:tradeTree.getVal,startDate:startDate,endDate:endDate};
