NB.ns("app.admin").hisPayDedailed = (function() {	
	...
	var hisPayDedailedMethodsBoxObj = $('#hisPayDedailedMethodsBox');
...
//   			   queryData(tableObj, queryObj);
	...
	function collect(orgNo){
		...
		var wnUrl = apiUrl+"/weinan/countSum";
		$.ajax({
	           url:url,
	           type:"get",
	           data:{
		...
		$.ajax({
	           ...
	           dataType:"json",
	           success:function(result){
	        	   putDataSumary(result);
	           }
	       });
	}
	function putDataSumary(recResult){
		let mzSumAmount = recResult.data.clinicAmountAdd + recResult.data.payAmountAdd +recResult.data.registerAmountAdd + recResult.data.makeAppointmentAmountAdd;
		hisPayDedailedMethodsBoxObj.find(".mzSumAmount").html(moneyFormat(mzSumAmount));//汇总金额
		hisPayDedailedMethodsBoxObj.find(".zySumAmount").html(moneyFormat(recResult.data.prepaymentForHospitalizationAmountAdd));//汇总缴费笔数
		hisPayDedailedMethodsBoxObj.find(".czAmount").html(moneyFormat(recResult.data.clinicAmountAdd));//汇总退费笔数
		hisPayDedailedMethodsBoxObj.find(".jfAmount").html(moneyFormat(recResult.data.payAmountAdd));//门诊金额
		hisPayDedailedMethodsBoxObj.find(".ghAmount").html(moneyFormat(recResult.data.registerAmountAdd + recResult.data.makeAppointmentAmountAdd));//门诊缴费笔数
	}
