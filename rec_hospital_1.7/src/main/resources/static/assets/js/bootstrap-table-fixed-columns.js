(function ($) {
  ...
  BootstrapTable.prototype.initBody = function () {
    ...
    this.$body.find('> tr[data-index]').each(function () {
      ...
      for (var i = 0; i < end; i++) {
        var fixtb = $tds.eq(i).clone();
        var bodytd = $(this).find('td').eq(i);
        if (fixtb.length > 0) {
          if (fixtb[0].childNodes.length > 0) {
            if (fixtb[0].childNodes[0].tagName == 'A') {
              var htmldiva = bodytd[0].childNodes;
              var fixhtmldiva = fixtb[0].childNodes;
              fixhtmldiva.forEach(function (item, key) {
                $(item).click(function () {
                  htmldiva[key].click();
                })
              })
  ...
  BootstrapTable.prototype.fitBodyColumns = function () {
    var that = this,
      ...
      height = this.$tableBody.height() - 12;
    ...
    if (!this.options.height) {
      top = this.$fixedHeader.height() - 0;
      height = height - top;
    }
    this.$fixedBody.css({
      width: this.$fixedHeader.width() + 2,
      height: height + 2,
      top: top + 1
    }).show();
    this.$body.find('> tr').each(function (i) {
      ...
      /*that.$fixedBody.find('tr:eq(3)').find('td').width($($(thattds).find('td')[3]).width()+1 );*/
    });
    // events
